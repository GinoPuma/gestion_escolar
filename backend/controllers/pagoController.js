...
exports.createPago = async (req, res) => {
  try {
    ...
    if (!matricula_id) return res.status(400).json({ message: "El ID de la matrícula es obligatorio." });
    if (!tipo_pago_id) return res.status(400).json({ message: "El ID del tipo de pago es obligatorio." });
    if (monto === undefined || monto === null) return res.status(400).json({ message: "El monto es obligatorio." });
    if (!fecha_pago) return res.status(400).json({ message: "La fecha de pago es obligatoria." });
    if (!estado) return res.status(400).json({ message: "El estado del pago es obligatorio." });
    ...
    if (metodo_pago_id !== null && metodo_pago_id !== undefined && metodo_pago_id !== '') { 
    ...
    const pagoData = {
      ...
      fecha_pago,
      metodo_pago_id: (metodo_pago_id === '' || metodo_pago_id === null || metodo_pago_id === undefined) ? null : parseInt(metodo_pago_id), 
      referencia_pago,
      estado,
    };
...
exports.updatePago = async (req, res) => {
  try {
    ...
    if (pagoData.metodo_pago_id !== undefined) {
        if (pagoData.metodo_pago_id !== null && pagoData.metodo_pago_id !== '' && pagoData.metodo_pago_id !== undefined) {
            const metodoPagoExists = await MetodoPago.getById(pagoData.metodo_pago_id);
            if (!metodoPagoExists) {
                return res.status(400).json({ message: "El método de pago especificado no existe." });
            }
        } else {
    ...
    if (pagoData.metodo_pago_id !== null && pagoData.metodo_pago_id !== undefined) {
